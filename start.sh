...
# 1. OpenClaw Mock (8082)
echo "[1/3] Starting OpenClaw Mock on :8082..."
$VENV adapter/openclaw_mock.py > /srv/openclaw/logs/mock.log 2>&1 &
OC_PID=$!
echo "  PID=$OC_PID"

# 2. Hermes Adapter (8081)
echo "[2/3] Starting Hermes Adapter on :8081..."
$VENV adapter/hermes_adapter.py > /srv/hermes/logs/adapter.log 2>&1 &
HM_PID=$!
echo "  PID=$HM_PID"
